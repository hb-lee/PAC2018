...
!----------------------------------------------------------------------
   ...
   if (my_task == master_task) then
      open (nml_in, file=nml_filename, status='old',iostat=nml_error)    !nml_filename = pop_in
      if (nml_error /= 0) then
         nml_error = -1
      else
         nml_error =  1
...
!----------------------------------------------------------------------
   ...
   tropic_distribution_type = 'cart_micmixed' ! 'cart_cpuonly'
   clinic_distribution_type = 'cart_micmixed'
   ...
   !if (my_task == master_task) write(6,*)'master_task work_per_block:',work_per_block
   !call MPI_BARRIER(MPI_COMM_OCN, ierr)
   !if (my_task == master_task+1) write(6,*)'master_task+1 work_per_block:',work_per_block
   !call MPI_BARRIER(MPI_COMM_OCN, ierr)
   !write(6,*)'nprocs_clinic:',nprocs_clinic
   !call MPI_BARRIER(MPI_COMM_OCN, ierr)
   !write(6,*)'nprocs_tropic:',nprocs_tropic
   !deallocate(work_per_block)
   !!xiaobin
   !call MPI_BARRIER(MPI_COMM_OCN, ierr)
   !call exit_POP(sigAbort,'domain test')
...
!----------------------------------------------------------------------
   ...
   call MPI_ALLREDUCE(nblocks_tmp_clinic,nblocks_max_clinic,1,MPI_INTEGER,MPI_MAX,MPI_COMM_OCN,ierr)
...
!----------------------------------------------------------------------
   ...
   call create_boundary(bndy_clinic, distrb_clinic, &
                        ...
                        trim(ew_boundary_type),     &
                        nx_global, ny_global)
write(6,*)'xiaobin init_domain_distribution1'
   call create_boundary(bndy_tropic, distrb_tropic, &
                        trim(ns_boundary_type),     &
                        trim(ew_boundary_type),     &
                        nx_global, ny_global)

write(6,*)'xiaobin init_domain_distribution2'
!----------------------------------------------------------------------
!EOC
